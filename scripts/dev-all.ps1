...
# 2. PlatformIO build
if (-not (Get-Command pio -ErrorAction SilentlyContinue)) { Write-Error 'PlatformIO (pio) não encontrado no PATH.' }
...
if (-not (Get-Command wokwi -ErrorAction SilentlyContinue)) {
  Write-Warning 'Wokwi CLI não encontrado. Execute scripts/install-wokwi-cli.ps1 antes.'
} else {
  $wokwiArgs = 'server'
